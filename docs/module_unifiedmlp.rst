...
* **training**: Lists of model properties at end of each epoch.
  ...
  * **accuracy**: List of lists. Per-class accuracy on validaton dataset.
  * **F1**: List of lists. Per-class F1 score on validaton dataset.
  * **loss_all**: List. Training loss on the training dataset.
  * **time_all**: List. Wallclock time elapsed in seconds per epoch.
...
* **performance**: Model properties after training.
  ...
  * **time_all**: Float. Average training time in seconds per epoch.
